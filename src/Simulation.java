...
public class Simulation {
    ...
    private static int daysSinceRelease = 0;
    ...
    private boolean contentShouldBeRequested(Content content) {
        ...
        if (content.getPopularityLevel().equals(veryPopularPopularityLevel) && randomChance <= VERY_POPULAR_CONTENT_CHANCE_OF_REQUEST_PERCENTAGE && content.getDaysSinceRelease() <= 7)
            return true;

        if (content.getPopularityLevel().equals(popularPopularityLevel) && randomChance <= POPULAR_CONTENT_CHANCE_OF_REQUEST_PERCENTAGE && content.getDaysSinceRelease() <= 14)
            return true;

        if (content.getPopularityLevel().equals(regularPopularityLevel) && randomChance <= REGULAR_CONTENT_CHANCE_OF_REQUEST_PERCENTAGE && content.getDaysSinceRelease() <= 21)
            return true;

        if (content.getPopularityLevel().equals(obsoletePopularityLevel) && randomChance <= OBSOLETE_CONTENT_CHANCE_OF_REQUEST_PERCENTAGE && content.getDaysSinceRelease() <= 30)
            return true;
    ...
    private void createContent(int i) throws Exception {
        Content content = new Content("Content " + i, getRandomDaysSinceRelease(), getRandomPopularityLevel());
    ...
    public int getRandomDaysSinceRelease() throws Exception {
        // get random number for days since release between 1 - 30
        daysSinceRelease = new Random().nextInt(30);

        return daysSinceRelease;

        // if (randomDaysSinceRelease < OBSOLETE_POPULAR_CONTENT_WEIGHT)
        //     // if random popularity < 10
        //     return randomDaysSinceRelease; //10
        // else if (randomDaysSinceRelease < OBSOLETE_POPULAR_CONTENT_WEIGHT + REGULAR_POPULAR_CONTENT_WEIGHT)
        //     // if random popularity < 85
        //     return randomDaysSinceRelease; //75
        // else if (randomDaysSinceRelease < OBSOLETE_POPULAR_CONTENT_WEIGHT + REGULAR_POPULAR_CONTENT_WEIGHT + POPULAR_CONTENT_WEIGHT)
        //     // if random popularity < 95
        //     return randomDaysSinceRelease; //10
        // else if (randomDaysSinceRelease < OBSOLETE_POPULAR_CONTENT_WEIGHT + REGULAR_POPULAR_CONTENT_WEIGHT + POPULAR_CONTENT_WEIGHT + VERY_POPULAR_CONTENT_WEIGHT)
        //     // if random popularity < 100
        //     return randomDaysSinceRelease; //5
        // else
        //     throw new Exception("Invalid days since release");
    }
    ...
    private PopularityLevel getRandomPopularityLevel() throws Exception {
        ...
        if (randomPopularityLevel < OBSOLETE_POPULAR_CONTENT_WEIGHT)
            ...
            // if random popularity < 100
            return veryPopularPopularityLevel; //5
        else
